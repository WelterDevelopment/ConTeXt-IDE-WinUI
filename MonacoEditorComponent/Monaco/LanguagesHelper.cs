...
namespace Monaco
{
    ...
    public sealed class LanguagesHelper
    {
        ...
        public IAsyncAction RegisterSignatureHelpProvider(string languageId, SignatureHelpProvider provider)
        {
            if (_editor.TryGetTarget(out CodeEditor editor))
            {
                // Wrapper around CompletionItem Provider to Monaco editor.
                // TODO: Add Incremented Id so that we can register multiple providers per language?
                editor._parentAccessor.RegisterEvent("SignatureHelpProvider" + languageId, async (args) =>
                {
                    if (args != null && args.Length >= 2)
                    {
                        var items = await provider.ProvideSignatureHelpAsync(editor.GetModel(), JsonConvert.DeserializeObject<Position>(args[0]), JsonConvert.DeserializeObject<SignatureHelpContext>(args[1]));
                ...
                return editor.ExecuteScriptAsync("registerSignatureHelpProvider", new object[] { languageId, provider.SignatureHelpTriggerCharacters, provider.SignatureHelpRetriggerCharacters }).AsAsyncAction();
            }
